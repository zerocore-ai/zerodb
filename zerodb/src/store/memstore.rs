...
mod tests {
    ...
    fn test_memstore_append_entries() -> anyhow::Result<()> {
        ...
        store.append_entries(vec![LogEntry {
            term: 1,
            command: Command::ClientRequest(Query::Get("a".to_string())),
        }])?;

        store.append_entries(vec![LogEntry {
            term: 1,
            command: Command::ClientRequest(Query::Get("b".to_string())),
        }])?;

        assert_eq!(store.entries.len(), 2);
        assert_eq!(store.get_last_term(), 1);
        assert_eq!(
            store.entries,
            vec![
                LogEntry {
